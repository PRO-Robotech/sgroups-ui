...
Bindings are matched by comparing `binding.spec.service` with the current Service `metadata.name` and `metadata.namespace`. Matching bindings are grouped by `spec.addressGroup.namespace`. Under each namespace node, each child shows the resolved AddressGroup label from `spec.displayName` or metadata.
