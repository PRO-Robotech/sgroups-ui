...
The modal title renders `Delete`, a canonical `Tenant` badge with the tenant display name when available, then a canonical `Network` badge with `spec.displayName` falling back to `metadata.name`.
