...
Host details add `Socket Stats` and `NFT` tabs. Both tabs read backend-owned Host subresources through the cluster proxy, default to `watch=true`, fetch one snapshot before opening the stream, and replace the full table on every streamed list snapshot. `Socket Stats` sends user-filled backend selectors; `NFT` sends only the backend-supported `watch` query knob. When structured nftables JSON is present, the NFT tab renders overview rows for tables, chains, rules, sets, and other nftables objects instead of showing raw ruleset text as the primary view.
